...
{$REGION 'Документация и обертка над dll'}
  ...
  TPayment_info_item = packed record
    dwTag: DWORD; // **< Тег платежной системы */
  ...
  TPreauthRec = packed record
    Amount: UINT;
    // вход: сумма предавторизации в копейках
  ...
  TAuthAnswer11 = packed record
    ...
    Hash: array [0 .. 40] of AnsiChar;
    // выход: хеш от номера карты, в формате ASCII с нулевым байтом в конце
    Track3: array [0 .. 103] of AnsiChar; // выход: третья дорожка карты
  end;
  ...
  *)
  ...
  TAuthAnswer12 = packed record
    ...
    Department: DWORD;
    (* * < [in] Порядковый Номер отдела от 0 до 14 - ти,
      включительно.При установке номера отдела в 0 xFFFFFFFF,
      Номер отдела будет запрошен через интерфейс терминала после вставки карты.
      Если Номер отдела будет указан вне настроенного диапазона,
      то терминал вернет Код ошибки 4191. *)
    RRN: array [0 .. 12] of AnsiChar;
    (* * < [in , out] Номер ссылки операции,
      присвоеный хостом.Используется Для операций возврат и множественной
      авторизации.Содержит Уникальный 12 -
      значный ссылочный Номер.При предавторизации это поле является выходным
      (его заполняет библиотека Pilot_nt.dll),
      а При завершении расчета – входным
      (значение должно быть заполнено вызывающей программой;
      оно должно совпадать со значением, возвращенным При предавторизации). *)
  end;
...
begin
  ...
  try
    try
      ...
      if FLibHandle <= 0 then
      begin
        raise TPinpadException.Create(Format('Библиотека %s не загружена', [LibName]));
        Exit;
      end;
      ...
      if not Assigned(Func) then
        raise TPinpadException.Create  ('Функция _card_authorize не найдена в pilot_nt.dll');
...
begin
  ...
  try
    try
      ...
      if not Assigned(Func) then
        raise TPinpadException.Create
          ('Функция _card_authorize12 не найдена в pilot_nt.dll');
...
//  PinpadLog(Format('Библиотека %s загружена', [LibName]));
//  TLogger.Info(Format('Библиотека %s загружена', [LibName]));
  inherited Create;
end;
...
begin
  case ErrorCode of
    - 1:
      Result := 'Неизвестная ошибка. См. лог';
    12:
...
begin
  ...
  try
    try
      ...
      if FTerminalID = '' then
        raise Exception.Create('Пинпад не инициализирован');
//      PinpadLog(Format('Терминал: %s', [FTerminalID]));
//      TLogger.Info(Format('Терминал: %s', [FTerminalID]));
...
begin
  ...
  try
    try
      ...
      if not Assigned(Func) then
        raise TPinpadException.Create
          ('Функция TestPinpad не найдена в pilot_nt.dll');
